...
class DAO {
  ...
  viewDeptUtilizedBudget = (id) => {
    return new Promise((resolve, reject) => {
      this.conn.query(
        ...
        join employee C on C.role_Id = B.id \
        where A.id = ? \
        group by A.id",
        id,
        function (err, results) {
  ...
  viewEmployeesByManager = (manager) => {
    return new Promise((resolve, reject) => {
      this.conn.query(
        "select A.first_name, A.last_name, IFNULL(CONCAT(B.first_name, ' ', B.last_name), '-') manager \
        from employee A \
  ...
  viewEmployeesByDepartment = (department) => {
    return new Promise((resolve, reject) => {
      this.conn.query(
        "select A.first_name, A.last_name, C.name department \
        from employee A \
        join role B on A.role_id = B.id \
