  ...
  async process(initAnswer) {
    switch (initAnswer.todo) {
      ...
      case "update_employee_role":
        ...
        questions.update_employee_role.push({
          type: "list",
          name: "employeeId",
        ...
        const { employeeId, update_employee_roleId } = await this.ask(
          questions.update_employee_role
        );
      ...
      case "update_employee_manager":
        ...
        questions.update_employee_manager.push({
          type: "list",
          name: "employeeId",
          message: "Select am employee",
        ...
        const answers = await this.ask(questions.update_employee_manager);
      ...
      case "view_employees_by_manager":
        const managers2 = await this.dao.viewAllManagers();

        questions.view_employees_by_manager.push({
          type: "list",
          name: "view_employees_by_manager_managerId",
          message: "Select a manager",
          choices: managers2.map((dbRec) => ({
            name: dbRec.first_name + " " + dbRec.last_name,
            value: `${dbRec.id}`,
          })),
        });

        const { view_employees_by_manager_managerId } = await this.ask(
          questions.view_employees_by_manager
        );

        console.log("\n"); // line break for visual spacing
        console.table(
          await this.dao.viewEmployeesByManager(
            view_employees_by_manager_managerId
          )
        );
        break;
      case "view_employees_by_department":
        const all_depts2 = await this.dao.viewAllDepartments();

        console.log(all_depts2);

        questions.view_employees_by_department.push({
          type: "list",
          name: "view_employees_by_department_departmentId",
          message: "Select department name",
          choices: all_depts2.map((dbRec) => ({
            name: dbRec.name,
            value: `${dbRec.id}`,
          })),
        });

        const { view_employees_by_department_departmentId } = await this.ask(
          questions.view_employees_by_department
        );

        console.log("\n"); // line break for visual spacing
        console.table(
          await this.dao.viewEmployeesByDepartment(
            view_employees_by_department_departmentId
          )
        );
        break;
      case "exit":
